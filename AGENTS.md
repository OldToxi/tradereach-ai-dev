...
| Service role | One file, `lib/supabase/admin.ts`, five callers: audit writes, Gmail token read/refresh, seed, reply-actions' read-only commercial-authority lookup (executives cannot read other profiles via RLS), and user-actions' user administration (invite creates an auth.user, and the team table reads last-sign-in) | Auditable boundary |
